...
def draw_from_dist(dist_config: dict, rng: np.random.Generator) -> float | int:
    """Reads the 'type' from the config and returns a value from the
...
def generate_constructors(
    const_config: dict, rng: np.random.Generator
) -> list[Constructor]:
    constructors = []
...
def generate_houses(
    ...
    for region, count in region_allocations:
        ...
        region_price_config = price_configs.get(
            region.name, price_configs.get("default_price")
        )
        ...
        for _ in range(count):
            ...
            houses.append(
                House(
                    ...
                    selling_price=round(
                        eval_price * 1.05, 2
                    ),  # Default asking price 5% above eval
...
def generate_households(
    hh_config: dict, regions: list[Region], rng: np.random.Generator
) -> list[Household]:
    households = []
    ...
    for archetype, count in zip(hh_config["archetypes"], arch_counts):
        for _ in range(count):
            ...
            for _ in range(archetype["adults"]):
                age = int(
                    rng.uniform(
                        archetype["adult_age_range"][0], archetype["adult_age_range"][1]
                    )
                )
...
def assign_housing_market(
    households: list[Household], houses: list[House], market_config: dict
) -> tuple[list[Household], list[House]]:
    ...
    for i, hh in enumerate(sorted_households):
        ...
        properties_to_buy = (
            market_config["max_investment_properties"] + 1 if is_investor else 1
        )
        ...
        for purchase_num in range(properties_to_buy):
            ...
            if is_primary:
                valid_choices = [
                    h for h in affordable if h.location.id == hh.preference.id
                ] or affordable
            ...
            if loan_amount > 0:
                mortgage = Loan(
                    id=uuid4(),
                    house=chosen_house,
                    remaining_morgtage=loan_amount,
                    paid_mortgage=downpayment,
                    rent=0.0,
                    interest_rate=0.04,
                    spread=0.015,
                    tax_type="fixed",
    ...
    available_for_rent = [
        h
        for h in houses
        if getattr(h, "forRent", False) or isinstance(h.owner, Constructor)
    ]
    ...
    for hh in renters:
        ...
        max_monthly_rent = (
            hh.income * market_config["max_rent_to_income_ratio"]
        ) / 12.0
...
def _generate_synthetic(config: dict) -> Scenario:
    ...
    households, houses = assign_housing_market(
        households, houses, config["market_initialization"]
    )

    return Scenario(
        regions=regions, households=households, houses=houses, constructors=constructors
    )
...
def build_and_export_scenario(
    ...
    if export_format == "json":
        ...
        # Pydantic's model_dump_json handles UUIDs, lists, and
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Synthetic Scenario Data for ABM Simulation"
    )
    ...
    try:
        ...
        print(
            "🚀 Your scenario is ready for the simulator engine at:\n"
...
if __name__ == "__main__":
    # how to run it:
    # python datageneration.py --config housing_crash_config.yaml --name
    # crash_scenario --format json
    main()
